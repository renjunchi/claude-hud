...
Write-Host "=== claude-hud uninstaller ===" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $MarketplacesFile) {
    ...
    $marketplaces.Remove("claude-hud-local")
    $marketplaces | ConvertTo-Json -Depth 10 | Set-Content $MarketplacesFile -Encoding UTF8
    Write-Host "Removed marketplace: claude-hud-local"
}
...
if (Test-Path $InstalledFile) {
    ...
    if ($installed.plugins) {
        $installed.plugins.Remove("claude-hud@claude-hud-local")
    }
    $installed | ConvertTo-Json -Depth 10 | Set-Content $InstalledFile -Encoding UTF8
    Write-Host "Removed plugin: claude-hud@claude-hud-local"
}

Write-Host ""
Write-Host "claude-hud uninstalled. Please restart Claude Code." -ForegroundColor Green
